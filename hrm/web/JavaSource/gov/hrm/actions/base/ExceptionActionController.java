...
/**
 * 
 * For Handling exceptions which are raised at runtime, It will log and send mail to the error reporting users of the application 
 * with the request parameters, session parameters and handlers
 *
 */
public class ExceptionActionController extends SimpleMappingExceptionResolver {
	...
	public ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex){
		...
		String subject = (ex.getLocalizedMessage() == null) ? (ex.getMessage() == null ? "Exception" : ex.getMessage()) : ex.getLocalizedMessage();
		...
		sb.append("<u><h3>Session Last Accessed Time </h3></u>").append(new Date(session.getLastAccessedTime())).append("<br/>");
